...
const { validate } = require("../../../../src/plugins/validation/oas3/semantic-validators/parameters")
...
describe("validation plugin - semantic - parameters - oas3", function() {
  ...
  it("should complain when `in` is missing", function() {
    ...
    expect(res.errors[0].message).toEqual("Parameters MUST have an `in` property.")
    expect(res.warnings.length).toEqual(0)
  })
  ...
  it("should complain when `in` is an invalid value", function() {
    ...
    const res = validate({ jsSpec: spec }, config)
    expect(res.errors.length).toEqual(1)
    expect(res.errors[0].path).toEqual(["paths", "/pets", "get", "parameters", "0", "in"])
  ...
  it("should complain when the parameter has an undescribed data type", function() {
    ...
    expect(res.errors.length).toEqual(1)
    expect(res.errors[0].path).toEqual(["paths", "/pets", "get", "parameters", "0"])
    expect(res.errors[0].message).toEqual("Parameters MUST have their data described by either `schema` or `content`.")
    expect(res.warnings.length).toEqual(0)
  })
  ...
  it("should complain when a parameter describes data type with both `schema` and `content`", function() {
    ...
    expect(res.errors[0].message).toEqual("Parameters MUST NOT have both a `schema` and `content` property.")
    expect(res.warnings.length).toEqual(0)
  })
