...
const constants = docs.filter(d => d.kind === "constant").map(
  c => {
    const members = docs.filter(d => (d.kind === "member" && d.memberof === c.longname));
...
const functions = docs.filter(d => d.kind === "function").map(
  f => ({
